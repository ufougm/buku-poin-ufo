...
export default function MentorDashboard() {
  ...
  // Find the mentor record for this user using stored mentorId
  const myMentor = useMemo(() => {
    const mockUser = user as any;
    if (mockUser?.mentorId) {
      return local.mentors.find((m) => m.id === Number(mockUser.mentorId)) || null;
    }
    return null;
  }, [user, local.mentors]);
  ...
  const mentees = useMemo(() => {
    if (!myMentor) return [];
    return local.getMenteesByMentor(myMentor.id);
  }, [myMentor, refreshTick]);

  // Get pending activities for mentees
  const pendingActivities = useMemo(() => {
    if (!myMentor || mentees.length === 0) return [];
  ...
  }, [myMentor, mentees, refreshTick]);

  // Count verified activities for mentees
  const verifiedCount = useMemo(() => {
    if (!myMentor || mentees.length === 0) return 0;
    const menteeIds = mentees.map((m) => m!.registrantId);
  ...
  const handleApprove = (activityId: number) => {
    local.updateActivity(activityId, { status: "verified", verifiedAt: new Date().toISOString(), verifiedBy: myMentor?.id });
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-sm text-gray-500">
            {myMentor ? `${myMentor.fullName} - Kelola dan verifikasi kegiatan calon anggota` : "Kelola dan verifikasi kegiatan calon anggota"}
          </p>
        </div>
